...
export const updateUser = async(req,res)=>{
    const {username, fullName, email, newPassword, currentPassword, link, bio} = req.body;
    ...
    try {
        ...
        if ((!newPassword && currentPassword) || (!currentPassword && newPassword)) {
            return res.status(404).json({error : "Plesae provide both newPassword & currentPassword"})
        }
        if (newPassword && currentPassword) {
            const isMatch =  becrypt.compare(currentPassword, user.password)

            if(!isMatch) return res.status(400).json({error : "Invalid current password"})
            if(newPassword.length < 6) {
                return res.status(400).json({
                    error : "Password must be at least 6 character log"
                })
            }

            const salt = await becrypt.genSalt(10)
            user.password = await becrypt.hash(newPassword, salt)
